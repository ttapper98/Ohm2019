...
vector<ohm_igvc_msgs::Target> ReadFile(string filename)
{
	...
	while(getline(file, str))
	{
		...
		if(lineFields.size() >= 2)
		{
			...
			// Set the frame ID for target
			currentTarget.header.frame_id = frameId;

			if(mode == "decimal")
			{
				currentTarget.latitude = atof(lineFields[0].c_str());
				currentTarget.longitude = atof(lineFields[1].c_str());
				currentTarget.heading = atof(lineFields[2].c_str());
			}
...
bool waypoint(ohm_igvc_srvs::waypoint::Request  &req,
              ohm_igvc_srvs::waypoint::Response &res){
...
int main(int argc, char **argv)
{
	...
	{
		ROS_FATAL("No navigation_file parameter specified!");
	}
